...
library(readr)
NYPD_Complaint_2022 <- read_csv("NYPD_Complaint_2022.csv")
```
```{r}
# Load libraries
library(tidyverse)
...
```{r}
# Victim_Race summary table 
race_summary_tabel <- complaint_2022 %>% 
  group_by(VIC_RACE) %>% 
  summarise(Count = n()) %>% 
  arrange(desc(Count))

gt(race_summary_tabel)
```
```{r}
