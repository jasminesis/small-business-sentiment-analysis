...
combined_reviews <- get(load("combined_reviews.RData"))
combined_reviews <- 
...
combined_reviews |>
  ...
  labs(title = "Average polarity over time for Asian, Mexican and pizza restaurants", color = "Cuisine", x = "Date", y = "Mean rating") + theme_minimal()
...
ggplot() +
  geom_point(data= pizza_avg_sentiment, aes(x=date,y=as.numeric(polarity),colour='one'),color="lightpink",show.legend = TRUE) + geom_smooth(data= pizza_avg_sentiment,aes(group=1,x=date,y=as.numeric(polarity)),color="lightpink", show.legend = TRUE, method="loess",se=FALSE) +
...
ggplot() +
  geom_point(data= pizza_avg_sentiment, aes(x=date,y=as.numeric(polarity)),color="lightpink",show.legend = TRUE) + scale_color_manual(
  values = c("lightpink"),
  labels = c("Pizza")
)
...
one_star_df_asian <- data.frame(date_lub = -435:446,
                          rating = 1)
...
one_star_df_mexican <- data.frame(date_lub = -435:446,
                          rating = 1)
...
one_star_df_pizza <- data.frame(date_lub = -435:446,
                          rating = 1)
...
ggplot(data= one_star_df, aes(x = date_lub, y = polarity, color = type)) +
  ...
  labs(title = "Polarity over time for 1 star reviews", color = "Cuisine", x = "Days from US COVID Lockdown", y = "Polarity") + theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) 
...
```{r}
asian_df <- data.frame(date_lub= -435:538,
                       rating = 5)
...
```{r}
asian_df2 <- data.frame(date_lub= rep(-435:446,each = 5),
                       rating = 1:5)
...
```{r}
mexican_df2 <- data.frame(date_lub= rep(-435:538,each = 5),
                       rating = 1:5)
...
```{r}
pizza_df2 <- data.frame(date_lub= rep(-435:538,each = 5),
                       rating = 1:5)
...
```{r}
reviews_1 <- data.frame(date_lub= -435:446,
                       rating = 1)
